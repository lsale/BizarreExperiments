...
SoundManager::SoundManager(QString soundDirectory) {
	...
	completeSoundDirectory =
			applicationDirectory.append("/app/native/assets/").append(
					soundDirectory);
	...
	if (!dir.exists()) {
		qDebug() << "Cannot find the sounds directory."
				<< completeSoundDirectory;
	} else {
		...
		for (int i = 0; i < list.size(); ++i) {
			...
			ALuint bufferID = alutCreateBufferFromFile(
...
ALint SoundManager::play(QString fileName, float pitch, float gain) {
	...
	if (bufferID != 0) {
		...
		source = mSoundSources[sourceIndex];
...
ALint SoundManager::play(QString fileName, float pitch) {
